  ...
  if(typeof Chart!=='undefined'){
    ...
    empPivotChartInst=new Chart(canvas,{
      ...
      options:{responsive:true,maintainAspectRatio:false,
        ...
        scales:{x:{stacked:true,ticks:{font:{size:10},color:_epTc},grid:{color:_epGc}},y:{stacked:true,ticks:{font:{size:10},color:_epTc,callback:function(v){return isCurrency?(v>=1e6?'$'+(v/1e6).toFixed(1)+'M':v>=1e3?'$'+(v/1e3).toFixed(0)+'K':'$'+v.toLocaleString()):v}},grid:{color:_epGc}}}
      }
    });
  }
