...
function createScratchPad(cfg){
  ...
  if(!grid){console.warn('Scratch pad: grid not found',cfg.grid);return null}
  ...
  if(!canvas){console.warn('Scratch pad: canvas not found',cfg.canvas);return null}
...
export function initVendorScratch(){
  ...
  else if(vendorScratch.resizeWbCanvas){vendorScratch.resizeWbCanvas()}
}
export function initDepScratch(){
  if(!depScratch){depScratch=createScratchPad(depScratchCfg)}
  else if(depScratch.resizeWbCanvas){depScratch.resizeWbCanvas()}
}
